...
class EbookUtils:
    ...
    def _get_mutool_path(self):
        """ 获取 mutool 路径 (跨平台) """
        if sys.platform == 'win32':
            # Windows 下使用内置工具
            path = os.path.join(os.path.join(os.path.join(self.settings.dir_bundle, 'tools'), 'MuPDF'), 'mutool.exe')
            if os.path.exists(path):
                return f'"{path}"'
            return "mutool.exe"
        else:
            # Linux 下使用系统安装的 mupdf-tools
            return "mutool"

    # ========== (〇) mdict-utils ==========
    def mdict(self, parms):
        """ 执行 mdict-utils 程序 """
        print(f"DEBUG: 执行 mdict-utils 参数: {parms}")
        try:
            # 使用 subprocess 调用 mdict-utils，防止 sys.exit() 杀死主进程
            result = subprocess.run([sys.executable, '-m', 'mdict_utils'] + parms, 
            ...
            if result.returncode != 0:
                print(Fore.RED + "ERROR: " + Fore.RESET + f"mdict-utils 执行失败 (返回码: {result.returncode})")
                return False
            return True
        except Exception as e:
            print(Fore.RED + "ERROR: " + Fore.RESET + f"调用 mdict-utils 时出错: {str(e)}")
            return False
    ...
    def export_mdx(self, mfile):
        ...
        if os.path.isfile(mfile) and mfile.endswith('.mdx'):
            ...
            if not self.mdict(['-x', mfile, '-d', out_dir]):
                done_flg = False
            if done_flg:
                ...
                with open(tmp_final_txt, 'w', encoding='utf-8') as fw:
                    ...
                    for line in fr:
                        ...
                        if (not link_flg) and re.match(r'</>\s*$', line):
                            ...
                            link_flg = False
                        fw.write(line)
            if not self.mdict(['--description', file_dict_info, '--encoding', 'utf-8', '-a', tmp_final_txt, ftitle+'.mdx']):
                mdx_flg = False
        else:
    ...
    def pdf_to_imgs(self, file_pdf, dir_out):
        ...
        # 判断是文字版还是图片版PDF
        img_pdf_flg = True
        mutool = self._get_mutool_path()
        os.system(f'{mutool} draw -o {tmp_txt} -F text "{file_pdf}" 2-11')
    ...
    def convert_pdf_to_imgs(self, file_pdf, dir_out, dpi=300):
        """ 使用 mutool 按 DPI 参数转换成图片 (推荐用于文字版PDF) """
        # 准备文件夹
        ...
        # 开始转换
        mutool = self._get_mutool_path()
        os.system(f'{mutool} draw -o "{file_png}" -F png -r {str(dpi)} "{file_pdf}"')
        print('转换完成！')
    ...
    def extract_pdf_to_imgs(self, file_pdf, dir_out):
        """ Extracting images with mutool (Supported on Windows/Linux) """
        ...
        os.chdir(dir_tmp_me)
        os.system(f'{mutool} extract "{file_pdf}"')
    ...
    def extract_pdf_to_imgs_pdfpatcher(self, file_pdf, dir_out):
        ...
        if not HAS_PYWINAUTO or sys.platform != 'win32':
            print(Fore.RED + "ERROR: " + Fore.RESET + "PDFPatcher 仅支持 Windows 环境")
            return
        # 0.配置程序选项
    ...
    def combine_img_to_pdf(self, dir_imgs, file_pdf):
        """ use mutool to combine images to pdf file (Supported on Windows/Linux) """
        # prepare paths
        mutool = self._get_mutool_path()
        dir_tmp = os.path.join(self.settings.dir_bundle, '_tmp')
        if not os.path.exists(dir_tmp):
        ...
        while k <= total_step:
            ...
            os.system(f'{mutool} create -o {tmp_pdf} -O compress-images {pcs_str}')
        ...
        while k <= total_step:
            ...
            os.system(f'{mutool} merge -o {tmp_pdf} {merge_str}')
            pdf_str = pdf_str + ' ' + tmp_pdf
            k += 1
        # output final single file
        os.system(f'{mutool} merge -o "{file_pdf}" {pdf_str}')
    ...
    def combine_img_to_pdf_fp2p(self, dir_imgs, file_pdf):
        ...
        if not HAS_PYWINAUTO or sys.platform != 'win32':
            print(Fore.RED + "ERROR: " + Fore.RESET + "FreePic2Pdf 仅支持 Windows 环境")
            return
        # 0.配置程序选项, 设定图像文件夹
    ...
    def convert_pdg_to_img(self, dir_pdg, dir_out):
        ...
        if not HAS_PYWINAUTO or sys.platform != 'win32':
            print(Fore.RED + "ERROR: " + Fore.RESET + "Pdg2Pic 仅支持 Windows 环境")
            return
        # 0.配置程序选项, 设定输出文件夹
    ...
    def eximport_bkmk_fp2p(self, file_pdf, file_bkmk_txt, mode=1):
